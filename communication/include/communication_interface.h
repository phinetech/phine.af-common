 ...
 */
 ...
 public:
     ...
     /**
      * @brief Register a REST endpoint handler (HTTP-specific)
      *
      * This method allows registering HTTP method + path combinations
      * for direct REST API endpoint handling. Only supported by HTTP transport.
      *
      * @param method HTTP method (GET, POST, DELETE, etc.)
      * @param path Path pattern (may include wildcards like * for parameters)
      * @param handler Handler function that receives HttpRequest and returns HttpServerResponse
      * @return bool True if registration succeeded, false if not supported by this transport
      */
     virtual bool register_http_endpoint(
         const std::string& method,
         const std::string& path,
         HttpRequestHandler handler) {
         return false; // Default: not supported by non-HTTP transports
     }
